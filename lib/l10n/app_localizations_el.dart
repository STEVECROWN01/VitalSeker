// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Modern Greek (`el`).
class AppLocalizationsEl extends AppLocalizations {
  AppLocalizationsEl([String locale = 'el']) : super(locale);

  @override
  String get appName => 'VitalSeker';

  @override
  String get tagline => 'Ο σύντροφός σας για την υγεία με τεχνητή νοημοσύνη';

  @override
  String get home => 'Αρχική';

  @override
  String get history => 'Ιστορικό';

  @override
  String get triage => 'Διαλογή';

  @override
  String get insights => 'Ευφυΐα';

  @override
  String get passport => 'Διαβατήριο';

  @override
  String get welcomeBack => 'Welcome Πίσω';

  @override
  String get signIn => 'Σύνδεση';

  @override
  String get signUp => 'Εγγραφή';

  @override
  String get signOut => 'Αποσύνδεση';

  @override
  String get email => 'Ηλεκτρονικό ταχυδρομείο';

  @override
  String get password => 'Κωδικός';

  @override
  String get fullName => 'Full Όνομα';

  @override
  String get confirmPassword => 'Επιβεβαίωση Κωδικός';

  @override
  String get createAccount => 'Δημιουργία λογαριασμού';

  @override
  String get dontHaveAccount => 'Δεν έχετε λογαριασμό;';

  @override
  String get alreadyHaveAccount => 'Έχετε ήδη λογαριασμό;';

  @override
  String get orContinueWith => 'ή συνεχίστε με';

  @override
  String get continueWithGoogle => 'Συνέχεια με το Google';

  @override
  String get continueWithApple => 'Συνέχεια με την Apple';

  @override
  String get forgotPassword => 'Forgot PΚωδικός';

  @override
  String get goodMorning => 'Καλημέρα';

  @override
  String get goodAfternoon => 'Καλησπέρα';

  @override
  String get goodEvening => 'Καλησπέρα';

  @override
  String get goodNight => 'Καληνύχτα';

  @override
  String get healthScore => 'Υγεία Score';

  @override
  String get quickActions => 'Γρήγορες ενέργειες';

  @override
  String get checkSymptomsNow => 'Check Συμπτώματα Now';

  @override
  String get healthPassport => 'Υγεία Διαβατήριο';

  @override
  String get myHistory => 'My Ιστορικό';

  @override
  String get emergencySOS => 'SOS έκτακτης ανάγκης';

  @override
  String get recentChecks => 'Πρόσφατοι έλεγχοι';

  @override
  String get noSymptomsLogs => 'Όχι Συμπτώματα logs yet';

  @override
  String get startTriage => 'Start Διαλογή';

  @override
  String get vitals => 'Ζωτικά σημεία';

  @override
  String get logVitals => 'Log Ζωτικά σημεία';

  @override
  String get heartRate => 'Σφύξεις';

  @override
  String get bloodPressure => 'Αρτηριακή πίεση';

  @override
  String get spo2 => 'SpO2';

  @override
  String get temperature => 'Θερμοκρασίες';

  @override
  String get weight => 'Βάρος';

  @override
  String get bloodGlucose => 'Γλυκόζη αίματος';

  @override
  String get respiratoryRate => 'Αναπνευστικός ρυθμός';

  @override
  String get aiTriage => 'AI Διαλογή';

  @override
  String get describeSymptoms => 'Describe your symΣυμπτώματα';

  @override
  String get analyzingSymptoms => 'Analyzing your Συμπτώματα';

  @override
  String get aiProcessing => 'AI is processing your Υγεία data';

  @override
  String get send => 'Αποστολή';

  @override
  String get newChat => 'Νέα συνομιλία';

  @override
  String get viewDetailedResults => 'Προβολή λεπτομερών αποτελεσμάτων';

  @override
  String get monitorAtHome => 'Monitor at Αρχική';

  @override
  String get whenToEscalate =>
      'Πότε πρέπει να αναφερθεί το θέμα σε ανώτερο επίπεδο';

  @override
  String get saveToPassport => 'Αποθήκευση to Διαβατήριο';

  @override
  String get shareResult => 'Κοινοποίηση Result';

  @override
  String get profile => 'Προφίλ';

  @override
  String get settings => 'Ρυθμίσεις';

  @override
  String get editProfile => 'Επεξεργασία Προφίλ';

  @override
  String get notifications => 'Ειδοποιήσεις';

  @override
  String get helpSupport => 'Βοήθεια & Υποστήριξη';

  @override
  String get about => 'Σχετικά';

  @override
  String get health => 'Υγεία';

  @override
  String get family => 'Οικογένεια';

  @override
  String get privacyPolicy => 'Απόρρητο Policy';

  @override
  String get termsOfService => 'Όροι of Service';

  @override
  String get deleteAccount => 'Διαγραφή Account';

  @override
  String get language => 'Γλώσσα';

  @override
  String get selectLanguage => 'Select Γλώσσα';

  @override
  String get appearance => 'Εμφάνιση';

  @override
  String get darkMode => 'Σκοτεινή λειτουργία';

  @override
  String get account => 'Λογαριασμός';

  @override
  String get privacyData => 'Απόρρητο & Data';

  @override
  String get securityStorage => 'Ασφάλεια και αποθήκευση';

  @override
  String get exportData => 'Εξαγωγή Data';

  @override
  String get support => 'Υποστήριξη';

  @override
  String get helpCenter => 'Βοήθεια Center';

  @override
  String get contactConcierge => 'Επικοινωνία με τον θυρωρό';

  @override
  String get aboutVitalSeker => 'Σχετικά VitalSeker';

  @override
  String get medicalRecords => 'Ιατρικά αρχεία';

  @override
  String get medicalID => 'Ιατρικό ID';

  @override
  String get medicalTranslation => 'Ιατρική μετάφραση';

  @override
  String get familyProfiles => 'Οικογένεια Profiles';

  @override
  String get addFamilyMember => 'Προσθήκη Οικογένεια Member';

  @override
  String get accountOwner => 'ΙΔΙΟΚΤΗΤΗΣ ΛΟΓΑΡΙΑΣΜΟΥ';

  @override
  String get upgradeToPro => 'Αναβάθμιση to Pro';

  @override
  String get protectWholeCircle => 'Προστάτεψε ολόκληρο τον κύκλο.';

  @override
  String get learnMore => 'Μάθετε περισσότερα';

  @override
  String get medications => 'Φάρμακα';

  @override
  String get addMedication => 'Προσθήκη Medication';

  @override
  String get appointments => 'Ραντεβού';

  @override
  String get addAppointment => 'Προσθήκη Appointment';

  @override
  String get upcoming => 'Προσεχείς εκδηλώσεις';

  @override
  String get completed => 'Ολοκληρώθηκε';

  @override
  String get cancelled => 'Ακυρώθηκε';

  @override
  String get active => 'Ενεργό';

  @override
  String get discontinued => 'Δεν διατίθεται πλέον';

  @override
  String get save => 'Αποθήκευση';

  @override
  String get cancel => 'Ακύρωση';

  @override
  String get delete => 'Διαγραφή';

  @override
  String get edit => 'Επεξεργασία';

  @override
  String get add => 'Προσθήκη';

  @override
  String get close => 'Κλείσιμο';

  @override
  String get retry => 'Επανάληψη';

  @override
  String get loading => 'LoaΦόρτωση';

  @override
  String poweredBy(String producer) {
    return 'Powered by Keter Marketing';
  }

  @override
  String get pressAndHold => 'Press and hold to send Επείγον alert';

  @override
  String get holdFor3Seconds => 'Κρατήστε για 3 δευτερόλεπτα';

  @override
  String get imSafeResolve => '«I\'m Safe» - Resolve';

  @override
  String get shareMyLocation => 'Κοινοποίηση My Τοποθεσία';

  @override
  String get findHospitalsNearMe => 'Βρες νοσοκομεία κοντά μου';

  @override
  String get quickDial => 'Γρήγορη κλήση';

  @override
  String get emergencyContacts => 'Επείγον Contacts';

  @override
  String get medicalIDSection => 'Ιατρικό ID';

  @override
  String get noEmergencyContacts => 'Όχι Επείγον contacts configured';

  @override
  String get addContactsInProfile =>
      'Προσθήκη contacts in your Προφίλ Ρυθμίσεις';

  @override
  String get noMedicalInfo => 'Όχι medical information on file';

  @override
  String get updateProfileMedicalID =>
      'Update your Προφίλ to Προσθήκη medical ID data';

  @override
  String get symptomHistory => 'Symptom Ιστορικό';

  @override
  String get noHistoryYet => 'Όχι Ιστορικό Yet';

  @override
  String get symptomLogsAppearHere =>
      'Τα αρχεία καταγραφής των συμπτωμάτων σας θα εμφανίζονται εδώ';

  @override
  String get startFirstTriage => 'Start Your First Διαλογή';

  @override
  String get weeklyInsights => 'Weekly Ευφυΐα';

  @override
  String get noInsightsYet => 'Όχι Ευφυΐα Yet';

  @override
  String get checkBackMonday =>
      'Όχι Ευφυΐα generated yet. Check Πίσω on Monday for your weekly AI Υγεία summary.';

  @override
  String get generateNow => 'Δημιουργία τώρα';

  @override
  String get upgradeProInsights =>
      'Αναβάθμιση to Pro to unlock AI-powered weekly Υγεία iΕυφυΐα';

  @override
  String get proPlan => 'Pro Plan';

  @override
  String get viewAllPlans => 'View Όλα plans';

  @override
  String get exportMedicalReport => 'Εξαγωγή Medical Report';

  @override
  String get generatePDF => 'Δημιουργία αρχείου PDF';

  @override
  String get sendByEmail => 'Αποστολή μέσω email';

  @override
  String get patientOverview =>
      'Συνοπτική παρουσίαση του ασθενούς και ζωτικά σημεία';

  @override
  String get symptomsTriageLog => 'Συμπτώματα & Διαλογή Log';

  @override
  String get medicationsAllergies => 'Φάρμακα & Αλλεργίες';

  @override
  String get aiAnalysisSummary => 'Σύνοψη ανάλυσης τεχνητής νοημοσύνης';

  @override
  String get dateRange => 'Ημερομηνία Range';

  @override
  String get last30Days => 'Τελευταίες 30 ημέρες';

  @override
  String get preview => 'ΠΡΟΕΠΙΣΚΟΠΗΣΗ';

  @override
  String get translate => 'Μετάφραση';

  @override
  String get translateMedicalTerms => 'Translate medical Όροι';

  @override
  String get targetLanguage => 'Target Γλώσσα';

  @override
  String get enterTextToTranslate =>
      'Εισάγετε ιατρικό όρο ή φράση προς μετάφραση';

  @override
  String translationResult(Object lang) {
    return 'Αποτέλεσμα μετάφρασης';
  }

  @override
  String get notificationPreferences =>
      'Οι προτιμήσεις σας όσον αφορά τις ειδοποιήσεις αποθηκεύονται στον λογαριασμό σας. Η δυνατότητα προσαρμογής του προγράμματος είναι πλέον διαθέσιμη — πατήστε οποιοδήποτε πρόγραμμα για να το αλλάξετε.';

  @override
  String get triageReminders => 'Διαλογή Reminders';

  @override
  String get medicationReminders => 'Υπενθυμίσεις για τη λήψη φαρμάκων';

  @override
  String get appointmentReminders => 'Υπενθυμίσεις ραντεβού';

  @override
  String get vitalsLoggingReminders => 'Ζωτικά σημεία Logging Reminders';

  @override
  String get healthTips => 'Υγεία Tips';

  @override
  String get weeklyReport => 'Εβδομαδιαία έκθεση';

  @override
  String get reminders => 'Υπενθυμίσεις';

  @override
  String get insightsTips => 'Ευφυΐα & Tips';

  @override
  String get thisActionIrreversible =>
      'This action is irreversible. Όλα your data will be permanently deleted.';

  @override
  String get typeEmailToConfirm => 'Type your Email to cΕπιβεβαίωση';

  @override
  String get deletePermanently => 'Διαγραφή Permanently';

  @override
  String get accountDeleted =>
      'Ο λογαριασμός διαγράφηκε. Λυπούμαστε που μας αφήνεις.';

  @override
  String get sendEmergencySOS => 'Send SOS έκτακτης ανάγκης?';

  @override
  String get sosMessageBody =>
      'This will send an SMS with your live Τοποθεσία to Όλα of your Επείγον contacts.';

  @override
  String get sendSOS => 'Αποστολή SOS';

  @override
  String get areYouSureSignOut => 'Είστε σίγουροι ότι θέλετε να αποσυνδεθείτε;';

  @override
  String get failedToSignOut =>
      'Η αποσύνδεση απέτυχε. Παρακαλώ, δοκιμάστε ξανά.';

  @override
  String get vitalsLogged => 'Ζωτικά σημεία Logged';

  @override
  String get triageSessions => 'Διαλογή Sessions';

  @override
  String get daysActive => 'Ημέρες δραστηριότητας';

  @override
  String get vitalSekerPro => 'VitalSeker Pro';

  @override
  String get signingOut => 'Αποσύνδεση...';

  @override
  String get manageMedicalCredentials => 'Διαχείριση ιατρικών πιστοποιητικών';

  @override
  String connectedMembers(num count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: '$count συνδεδεμένα μέλη',
      one: '$count συνδεδεμένο μέλος',
    );
    return '$_temp0';
  }

  @override
  String get alertsSmartReminders => 'Ειδοποιήσεις και έξυπνες υπενθυμίσεις';

  @override
  String get documentsImaging => 'Έγγραφα και απεικόνιση';

  @override
  String get translateMedicalTermsSubtitle => 'Translate medical Όροι';

  @override
  String get emergencyMedicalCard => 'Επείγον medical card';

  @override
  String get aes256EncryptionActive => 'Η κρυπτογράφηση AES-256 είναι ενεργή';

  @override
  String get downloadYourHealthData => 'Download your Υγεία data';

  @override
  String get themePasswordAccount => 'Theme, pΚωδικός account';

  @override
  String get faqsDocumentation => 'Συχνές ερωτήσεις και τεκμηρίωση';

  @override
  String get priorityProSupport => 'Priority Pro Υποστήριξη';

  @override
  String aboutVitalSekerVersion(String version) {
    return 'Έκδοση $version';
  }

  @override
  String get changePassword => 'Change Κωδικός';

  @override
  String get currentPassword => 'Current Κωδικός';

  @override
  String get newPassword => 'New Κωδικός';

  @override
  String get confirmNewPassword => 'Επιβεβαίωση New Κωδικός';

  @override
  String get update => 'Ενημέρωση';

  @override
  String get passwordsDoNotMatch => 'Οι κωδικοί πρόσβασης δεν ταιριάζουν';

  @override
  String get passwordMinLength => 'Κωδικός must be at least 8 characters';

  @override
  String get passwordUpdatedSuccessfully => 'Κωδικός updated successfully';

  @override
  String get failedToUpdatePassword =>
      'Η ενημέρωση του κωδικού πρόσβασης απέτυχε. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get deleteAccountIrreversible =>
      'This action is irreversible. Όλα your data — vΖωτικά σημεία mΦάρμακα appointments, symptom logs, Οικογένεια profiles, and Υγεία Διαβατήριο — will be permanently deleted.';

  @override
  String get emailDoesNotMatch => 'Η διεύθυνση email δεν ταιριάζει.';

  @override
  String get failedToDeleteAccount =>
      'Δεν κατέστη δυνατή η διαγραφή του λογαριασμού. Παρακαλώ δοκιμάστε ξανά ή επικοινωνήστε με την υποστήριξη.';

  @override
  String get manageYourSubscription => 'Διαχείριση της συνδρομής σας';

  @override
  String get permanentlyRemoveYourData => 'Permanently Αφαίρεση your data';

  @override
  String get endYourCurrentSession => 'Τερματίστε την τρέχουσα συνεδρία σας';

  @override
  String get dark => 'Σκοτεινό';

  @override
  String get light => 'Φως';

  @override
  String get systemDefault => 'Προεπιλογή συστήματος';

  @override
  String get subscription => 'Συνδρομή';

  @override
  String get restore => 'Επαναφορά';

  @override
  String get chooseYourPlan => 'Επιλέξτε το πακέτο σας';

  @override
  String get unlockFullPower =>
      'Αξιοποιήστε πλήρως τις δυνατότητες του VitalSeker';

  @override
  String get paymentIntegrationPending =>
      'Η ενσωμάτωση της δυνατότητας πληρωμών μέσα από την εφαρμογή (RevenueCat / StoreKit) βρίσκεται σε εκκρεμότητα. Οι αλλαγές στο πακέτο εφαρμόζονται απευθείας στον λογαριασμό σας για δοκιμαστικούς σκοπούς.';

  @override
  String get forever => 'για πάντα';

  @override
  String get perMonth => '/μήνα';

  @override
  String get freePlanFeature1 => '3 AI Διαλογή sessions/month';

  @override
  String get freePlanFeature2 => 'Basic Υγεία Διαβατήριο';

  @override
  String get freePlanFeature3 => 'Κοινή χρήση κωδικού QR';

  @override
  String get freePlanFeature4 => 'SOS έκτακτης ανάγκης alerts';

  @override
  String get freePlanFeature5 => 'Single user Προφίλ';

  @override
  String get proPlanFeature1 => 'Unlimited AI Διαλογή sessions';

  @override
  String get proPlanFeature2 => 'Advanced Υγεία Διαβατήριο';

  @override
  String get proPlanFeature3 => 'Weekly AI Ευφυΐα';

  @override
  String get proPlanFeature4 => 'Οικογένεια profiles (up to 5)';

  @override
  String get proPlanFeature5 => 'PDF Εξαγωγή with full Ιστορικό';

  @override
  String get proPlanFeature6 => 'Priority Υποστήριξη';

  @override
  String get enterprisePlanFeature1 => 'Όλα στο Pro';

  @override
  String get enterprisePlanFeature2 => 'Unlimited Οικογένεια profiles';

  @override
  String get enterprisePlanFeature3 => 'Προσαρμοσμένη επωνυμία';

  @override
  String get enterprisePlanFeature4 => 'Πρόσβαση στο API';

  @override
  String get enterprisePlanFeature5 => 'Dedicated Υποστήριξη';

  @override
  String get enterprisePlanFeature6 => 'Εγγύηση SLA';

  @override
  String get bestValue => 'ΚΑΛΥΤΕΡΗ ΣΧΕΣΗ ΠΟΙΟΤΗΤΑΣ-ΤΙΜΗΣ';

  @override
  String get currentPlan => 'Τρέχον σχέδιο';

  @override
  String get downgrade => 'Υποβάθμιση';

  @override
  String upgradeToPlan(String plan) {
    return 'Αναβάθμιση to $plan';
  }

  @override
  String get mustBeSignedInToChangePlans =>
      'Πρέπει να έχετε συνδεθεί για να αλλάξετε πακέτο.';

  @override
  String switchToPlan(String plan) {
    return 'Θέλετε να μεταβείτε στο $plan?';
  }

  @override
  String get downgradeToFreeMessage =>
      'Θα χάσετε την πρόσβαση στις λειτουργίες της έκδοσης Pro στο τέλος της τρέχουσας περιόδου χρέωσης. Θέλετε να συνεχίσετε;';

  @override
  String upgradeToPlanMessage(String plan) {
    return 'Με αυτόν τον τρόπο θα ενημερωθεί η συνδρομή σας στο $plan. Σε περιβάλλον παραγωγής, αυτό θα ενεργοποιούσε το paywall της πλατφόρμας· προς το παρόν, η αλλαγή εφαρμόζεται απευθείας στον λογαριασμό σας για δοκιμαστικούς σκοπούς.';
  }

  @override
  String get confirm => 'Επιβεβαίωση';

  @override
  String get downgradedToFree =>
      'Downgraded to FΔωρεάν Pro access ends at the Επόμενο billing period.';

  @override
  String welcomeToPlan(String plan) {
    return 'Welcome to $plan! Όλα features unlocked.';
  }

  @override
  String get failedToUpdateSubscription =>
      'Δεν κατέστη δυνατή η ενημέρωση της συνδρομής. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get purchasesRestored => 'Οι αγορές αποκαταστάθηκαν.';

  @override
  String get failedToRestorePurchases =>
      'Δεν κατέστη δυνατή η επαναφορά των αγορών.';

  @override
  String get contactSalesEnterprise =>
      'Contact sales for custom Enterprise Όροι';

  @override
  String get emailSalesEnterprise =>
      'Email sales@vitalseker.com for Enterprise pricing.';

  @override
  String poweredByProducer(String producer) {
    return 'Powered by $producer';
  }

  @override
  String get frequentlyAskedQuestions => 'Συχνές ερωτήσεις';

  @override
  String get contactSupport => 'Contact Υποστήριξη';

  @override
  String get subject => 'Θέμα';

  @override
  String get message => 'Μήνυμα';

  @override
  String get submit => 'Υποβολή';

  @override
  String get otherWaysToReachUs => 'Άλλο Ways to Reach Us';

  @override
  String get emailUs => 'Στείλτε μας email';

  @override
  String get supportRequestSaved =>
      'Your request is saved to your account and visible to our Υποστήριξη team. We respond within 24 hours.';

  @override
  String get pleaseFillSubjectMessage =>
      'Παρακαλώ συμπληρώστε τόσο το θέμα όσο και το μήνυμα.';

  @override
  String get subjectMinLength =>
      'Το θέμα πρέπει να έχει τουλάχιστον 5 χαρακτήρες.';

  @override
  String get messageMinLength =>
      'Το μήνυμα πρέπει να έχει τουλάχιστον 10 χαρακτήρες.';

  @override
  String get mustBeSignedInToSubmitSupport =>
      'You must be signed in to submit a Υποστήριξη request.';

  @override
  String get urgentRequestReceived =>
      'Λάβαμε επείγουσα αίτηση! Η ομάδα μας θα της δώσει προτεραιότητα.';

  @override
  String get supportRequestSent =>
      'Υποστήριξη request sent! We\'ll respond within 24 hours.';

  @override
  String get failedToSubmitSupport =>
      'Δεν κατέστη δυνατή η υποβολή του αιτήματος υποστήριξης. Παρακαλούμε δοκιμάστε ξανά ή στείλτε email στη διεύθυνση support@vitalseker.com.';

  @override
  String get couldNotOpenEmailClient =>
      'Could not open Email client. Please Email support@vitalseker.com manually.';

  @override
  String get faqQuestion1 => 'How does the AI symptom Διαλογή work?';

  @override
  String get faqAnswer1 =>
      'Our AI analyzes your reported Συμπτώματα against a comprehensive medical database to provide urgency-based recommendations. It categorizes your condition into Low, Medium, High, or Επείγον urgency levels and suggests appropriate Επόμενο steps.';

  @override
  String get faqQuestion2 => 'Is my Υγεία data secure?';

  @override
  String get faqAnswer2 =>
      'YΝαι Όλα data is encrypted end-to-end using AES-256 encryption. We comply with GDPR and HIPAA standards. Your Υγεία information is never shared with third parties without your explicit consent.';

  @override
  String get faqQuestion3 => 'How do I Κοινοποίηση my Υγεία pΔιαβατήριο';

  @override
  String get faqAnswer3 =>
      'Navigate to your Υγεία Διαβατήριο from the bottom navigation bar. Tap the QR code icon to generate a shareable QR code that healthcare providers can scan to access your critical Υγεία information securely.';

  @override
  String get faqQuestion4 => 'Can I Προσθήκη Οικογένεια members?';

  @override
  String get faqAnswer4 =>
      'YΝαι Pro subscribers can Προσθήκη up to 5 Οικογένεια member profiles, and Enterprise subscribers have unlimited Οικογένεια profiles. Each Οικογένεια member gets their own Υγεία Διαβατήριο and Διαλογή capabilities.';

  @override
  String get faqQuestion5 => 'How do I Ακύρωση my subscription?';

  @override
  String get faqAnswer5 =>
      'Μεταβείτε στο «Προφίλ» > «Συνδρομή» και επιλέξτε το πακέτο «Δωρεάν» για να κάνετε υποβάθμιση. Οι λειτουργίες του πακέτου Pro ή Enterprise θα παραμείνουν ενεργές μέχρι το τέλος της τρέχουσας περιόδου χρέωσης.';

  @override
  String get exportConfigurePreview =>
      'Configure and preview your comprehensive Υγεία summary before generating a secure PDF.';

  @override
  String get includeSections => 'Συμπερίληψη ενοτήτων';

  @override
  String get last3Months => 'Τελευταίοι 3 μήνες';

  @override
  String get yearToDate => 'Year to Ημερομηνία';

  @override
  String get allTime => 'Όλα Ώρα';

  @override
  String get generating => 'Δημιουργία…';

  @override
  String get sending => 'Αποστολή…';

  @override
  String pdfIncludesProducer(String producer) {
    return 'Το PDF περιλαμβάνει την αναφορά του $producer ως παραγωγού';
  }

  @override
  String get proFeature => 'Pro FEATURE';

  @override
  String get proActive => 'Pro ACTIVE';

  @override
  String get manageHealthWholeFamily =>
      'Manage Υγεία for your whole Οικογένεια (5 max)';

  @override
  String get accountOwnerDefault => 'Κάτοχος λογαριασμού';

  @override
  String get ownerProfile => 'Owner Προφίλ';

  @override
  String years(int age) {
    return '$age χρόνια';
  }

  @override
  String scoreValue(int score) {
    return 'Βαθμολογία: $score';
  }

  @override
  String get reachedProLimit => 'Έχετε φτάσει το όριο των 5 μελών του Pro.';

  @override
  String get limitReached => 'Επιτεύχθηκε το όριο';

  @override
  String get pleaseFillNameRelationship =>
      'Please fill in Όνομα and relationship';

  @override
  String get mustBeSignedInToAddFamily =>
      'You must be signed in to Προσθήκη a Οικογένεια member';

  @override
  String get familyMemberAdded => 'Οικογένεια member added!';

  @override
  String get failedToAddFamily =>
      'Δεν κατέστη δυνατή η προσθήκη μέλους της οικογένειας. Παρακαλώ, δοκιμάστε ξανά.';

  @override
  String get removeFamilyMember => 'Αφαίρεση Οικογένεια Member';

  @override
  String removeFamilyMemberConfirm(String name) {
    return 'Είσαι σίγουρος ότι θέλεις να διαγράψεις το $name από τα προφίλ της οικογένειάς σου;';
  }

  @override
  String get remove => 'Αφαίρεση';

  @override
  String get familyMemberRemoved => 'Οικογένεια member removed';

  @override
  String get failedToRemoveFamily =>
      'Δεν κατέστη δυνατή η διαγραφή του μέλους της οικογένειας. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get fullNameLabel => 'Full Όνομα';

  @override
  String get relationshipExample => 'Σχέση (π.χ. σύζυγος, παιδί)';

  @override
  String get bloodTypeOptional => 'Ομάδα αίματος (optional)';

  @override
  String get removeMember => 'Αφαίρεση member';

  @override
  String get upgradeYourCare => 'Αναβάθμιση YOUR CARE';

  @override
  String get protectingWholeCircle => 'Προστατεύεις ολόκληρο τον κύκλο.';

  @override
  String get proMemberThanks =>
      'Thanks for being a Pro member. You can monitor heart rate variability, sleep patterns, and AI-driven Υγεία risk assessments for up to 5 Οικογένεια members under a single subscription.';

  @override
  String get proUpsellBody =>
      'With VitalSeker PPro you can monitor heart rate variability, sleep patterns, and AI-driven Υγεία risk assessments for up to 5 Οικογένεια members under a single subscription.';

  @override
  String upgradeToProPrice(String price) {
    return 'Αναβάθμιση to Pro — \$$price/mo';
  }

  @override
  String get manageSubscription => 'Διαχείριση συνδρομής';

  @override
  String get failedToLoadProfiles => 'Δεν κατέστη δυνατή η φόρτωση των προφίλ';

  @override
  String get searchMedications => 'Αναζήτηση medΦάρμακα';

  @override
  String get noMedicationsMatchSearch => 'Όχι Φάρμακα match your Αναζήτηση';

  @override
  String get noMedicationsYet => 'Όχι Φάρμακα Yet';

  @override
  String get addMedicationsTrack =>
      'Προσθήκη your Φάρμακα to track dosages, frequency, and adherence';

  @override
  String editMedicationName(String name) {
    return 'Επεξεργασία $name';
  }

  @override
  String get dosage => 'Δοσολογία';

  @override
  String get unit => 'Μονάδα';

  @override
  String get frequency => 'Συχνότητα';

  @override
  String get notes => 'Σημειώσεις';

  @override
  String get medicationDiscontinued => 'Διακοπή της φαρμακευτικής αγωγής';

  @override
  String get failedToDiscontinueMedication =>
      'Δεν κατάφερε να διακόψει τη λήψη των φαρμάκων.';

  @override
  String get medicationUpdated => 'Ενημέρωση φαρμακευτικής αγωγής!';

  @override
  String get failedToUpdateMedication =>
      'Δεν κατέστη δυνατή η ενημέρωση των φαρμάκων.';

  @override
  String get medicationMarkedCompleted =>
      'Φάρμακο που έχει επισημανθεί ως ολοκληρωμένο';

  @override
  String get deleteMedication => 'Διαγραφή Medication';

  @override
  String deleteMedicationConfirm(String name) {
    return 'Είσαι σίγουρος ότι θέλεις να διαγράψεις το $name?';
  }

  @override
  String get medicationDeleted => 'Το φάρμακο διαγράφηκε';

  @override
  String get failedToDeleteMedication =>
      'Δεν κατέστη δυνατή η διαγραφή του φαρμάκου.';

  @override
  String get editDetails => 'Επεξεργασία Details';

  @override
  String get markComplete => 'Σημείωση: Ολοκλήρωση';

  @override
  String get discontinue => 'Διακοπή';

  @override
  String nextDose(String time) {
    return 'Επόμενη δόση: $time';
  }

  @override
  String get onceDaily => 'Μία φορά την ημέρα';

  @override
  String get twiceDaily => 'Δύο φορές την ημέρα';

  @override
  String get threeTimesDaily => 'Τρεις φορές την ημέρα';

  @override
  String get fourTimesDaily => 'Τέσσερις φορές την ημέρα';

  @override
  String get everyOtherDay => 'Every Άλλο Day';

  @override
  String get weekly => 'Εβδομαδιαία';

  @override
  String get asNeeded => 'Όποτε απαιτείται';

  @override
  String get custom => 'Προσαρμοσμένο';

  @override
  String get all => 'Όλα';

  @override
  String get noAppointmentsMatchFilter => 'Όχι appointments match the Φίλτρο';

  @override
  String get noAppointmentsYet => 'Όχι Appointments Yet';

  @override
  String get scheduleFirstAppointment =>
      'Schedule your first appointment to keep track of visits';

  @override
  String get appointmentMarkedCompleted =>
      'Το ραντεβού έχει επισημανθεί ως ολοκληρωμένο';

  @override
  String get failedToUpdateAppointment =>
      'Δεν κατέστη δυνατή η ενημέρωση του ραντεβού.';

  @override
  String get appointmentCancelled => 'Το ραντεβού ακυρώθηκε';

  @override
  String get failedToCancelAppointment =>
      'Δεν κατέστη δυνατή η ακύρωση του ραντεβού.';

  @override
  String get selectNewDate => 'Select new Ημερομηνία';

  @override
  String get selectNewTime => 'Select new Ώρα';

  @override
  String rescheduledTo(String date, String time) {
    return 'Αναπρογραμματίστηκε για την $time στις $date';
  }

  @override
  String get failedToRescheduleAppointment =>
      'Δεν κατέστη δυνατή η αλλαγή της ημερομηνίας του ραντεβού.';

  @override
  String get deleteAppointment => 'Διαγραφή Appointment';

  @override
  String deleteAppointmentConfirm(String doctor) {
    return 'Are you sure you want to Διαγραφή the appointment with $doctor?';
  }

  @override
  String get appointmentDeleted => 'Το ραντεβού διαγράφηκε';

  @override
  String get failedToDeleteAppointment =>
      'Δεν κατέστη δυνατή η διαγραφή του ραντεβού.';

  @override
  String get reschedule => 'Αλλαγή ημερομηνίας';

  @override
  String get cancelAppointment => 'Ακύρωση Appointment';

  @override
  String get medicalTranslationIntro =>
      'Translate medical Όροι and phrases into your preferred lΓλώσσα Useful for travel, consultations, and discussing care with non-English-speaking providers.';

  @override
  String get medicalTermOrPhrase => 'Ιατρικός όρος ή φράση';

  @override
  String get medicalTermHint =>
      'π.χ. «υπέρταση», «να λαμβάνεται δύο φορές την ημέρα μαζί με το φαγητό»';

  @override
  String get translating => 'Μετάφραση...';

  @override
  String translationTargetLanguage(String lang) {
    return 'Μετάφραση ($lang)';
  }

  @override
  String get translationWillAppear => 'Η μετάφρασή σας θα εμφανιστεί εδώ.';

  @override
  String get pleaseEnterTermToTranslate =>
      'Παρακαλώ εισάγετε έναν ιατρικό όρο ή μια φράση για μετάφραση.';

  @override
  String get noTranslationReturned =>
      'Όχι translation was returned. Please try a different term.';

  @override
  String get translationFailed =>
      'Η μετάφραση απέτυχε. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get notificationSettings => 'Notification Ρυθμίσεις';

  @override
  String get failedToSaveNotificationSetting =>
      'Δεν κατέστη δυνατή η αποθήκευση της ρύθμισης ειδοποιήσεων';

  @override
  String get keyFeatures => 'Βασικά χαρακτηριστικά';

  @override
  String get producer => 'Παραγωγός';

  @override
  String get conceptDesignDevelopment => 'Ιδέα, Σχεδιασμός & Ανάπτυξη';

  @override
  String get updateAccountCredentials =>
      'Ενημερώστε τα στοιχεία σύνδεσης του λογαριασμού σας';

  @override
  String get legal => 'Legal';

  @override
  String get aboutVitalSekerBody =>
      'VitalSeker is your AI-powered Υγεία companion that puts you in control of your Υγεία journey. With intelligent symptom tΔιαλογή a secure Υγεία pΔιαβατήριο Επείγον SOS alerts, and personalized weekly iΕυφυΐα VitalSeker ensures you always have the information you need when it matters most. Built with cutting-edge AI technology and bank-grade security, your Υγεία data stays private and protected.';

  @override
  String get featureAiTriageTitle => 'AI Symptom Διαλογή';

  @override
  String get featureAiTriageDesc =>
      'Get instant AI-powered Υγεία recommendations';

  @override
  String get featureHealthPassportTitle => 'Υγεία Διαβατήριο';

  @override
  String get featureHealthPassportDesc =>
      'Carry your encrypted Υγεία Προφίλ everywhere';

  @override
  String get featureQrSharingTitle => 'Κωδικός QR Sharing';

  @override
  String get featureQrSharingDesc =>
      'Κοινοποίηση Υγεία info securely with any provider';

  @override
  String get featureEmergencySosTitle => 'SOS έκτακτης ανάγκης';

  @override
  String get featureEmergencySosDesc =>
      'One-tap alerts with GPS Τοποθεσία sharing';

  @override
  String get featureWeeklyInsightsTitle => 'Weekly Ευφυΐα';

  @override
  String get featureWeeklyInsightsDesc => 'AI-generated Υγεία summaries (PPro';

  @override
  String get featureFamilyProfilesTitle => 'Οικογένεια Profiles';

  @override
  String get featureFamilyProfilesDesc =>
      'Manage Υγεία for your entire Οικογένεια';

  @override
  String get featurePdfExportTitle => 'PDF Εξαγωγή';

  @override
  String get featurePdfExportDesc => 'Generate and Κοινοποίηση Υγεία reports';

  @override
  String termsOfServiceTitle(String appName) {
    return '$appName Όροι of Service';
  }

  @override
  String lastUpdatedVersion(String version) {
    return 'Τελευταία ενημέρωση: Έκδοση $version';
  }

  @override
  String get tosSection1Title => 'Acceptance of Όροι';

  @override
  String tosSection1Para1(String appName) {
    return 'By creating an account, accessing, or using the $appName mobile application (\"the Service\"), you agree to be bound by these Όροι of Service (\"TerΌροι If you do not agree to these TΌροι you must not access or use the Service.';
  }

  @override
  String tosSection1Para2(String producer) {
    return 'The Service is provided by $producer (\"we\", \"us\", or \"our\"). These Όροι form a legally binding agreement between you and us.';
  }

  @override
  String get tosSection2Title => 'Δικαίωμα συμμετοχής & Λογαριασμός';

  @override
  String get tosSection2Para1 =>
      'You must be at least 13 years old to use the Service. If you are under 18, you represent that your parent or legal guardian has read and agreed to these Όροι on your behalf.';

  @override
  String get tosSection2Para2 =>
      'You are responsible for maintaining the confidentiality of your account credentials and for Όλα activities that occur under your account. Notify us immediately of any unauthorized use of your account.';

  @override
  String get tosSection3Title => 'Υγεία Information — Not Medical Advice';

  @override
  String tosSection3Para1(String appName) {
    return '$appName is a Υγεία companion application intended for informational and organizational purposes only. The Service is NOT a medical device and does not provide medical advice, diagnosis, or treatment recommendations.';
  }

  @override
  String get tosSection3Para2 =>
      'The AI Διαλογή feature provides general guidance based on the Συμπτώματα you report. It is not a substitute for professional medical judgment. Always seek the advice of a qualified healthcare provider with any questions you may have regarding a medical condition. Never disregard professional medical advice or delay seeking it because of something you read in this Service.';

  @override
  String get tosSection3Para3 =>
      'In a medical eΕπείγον call your local Επείγον number (e.g. 911, 112) immediately. Do not rely on the Service for Επείγον response.';

  @override
  String get tosSection4Title => 'Χρήση της Υπηρεσίας';

  @override
  String get tosSection4Intro => 'Συμφωνείτε να ΜΗΝ:';

  @override
  String get tosSection4Bullet1 =>
      'Να χρησιμοποιείτε την Υπηρεσία για οποιονδήποτε παράνομο σκοπό·';

  @override
  String get tosSection4Bullet2 =>
      'Να επιχειρήσετε να προβείτε σε αντίστροφη μηχανική, αποσυμπίληση ή αποσυναρμολόγηση της εφαρμογής·';

  @override
  String get tosSection4Bullet3 =>
      'Να ανεβάζετε περιεχόμενο που είναι κακόβουλο, απατηλό ή παραβιάζει τα δικαιώματα πνευματικής ιδιοκτησίας·';

  @override
  String get tosSection4Bullet4 =>
      'Interfere with the proper functioning of the Service or attempt to access data belonging to Άλλο users;';

  @override
  String get tosSection4Bullet5 =>
      'Να χρησιμοποιείτε την Υπηρεσία για την αποστολή ανεπιθύμητων μηνυμάτων ή spam.';

  @override
  String get tosSection5Title => 'Συνδρομές και πληρωμές';

  @override
  String get tosSection5Para1 =>
      'Certain features of the Service require a paid subscription (\"PPro or \"EEnterprise plan). Subscription fees are billed monthly through the platform application store (Apple App Store or Google Play Store) subject to their respective tΌροι';

  @override
  String get tosSection5Para2 =>
      'Subscriptions automatically renew unless cancelled at least 24 hours before the end of the current billing period. You can manage or Ακύρωση your subscription at any Ώρα through your platform\'s account sΡυθμίσεις';

  @override
  String get tosSection5Para3 =>
      'Ενδέχεται να τροποποιήσουμε τα τέλη συνδρομής κατόπιν εύλογης προειδοποίησης. Οι αλλαγές στα τέλη δεν θα ισχύουν για την τρέχουσα περίοδο χρέωσής σας.';

  @override
  String get tosSection6Title => 'Τα δεδομένα σας';

  @override
  String get tosSection6Para1 =>
      'You retain ownership of the Υγεία data you submit to the Service. Our use of your data is described in our Απόρρητο Policy, which is incorporated into these Όροι by reference.';

  @override
  String get tosSection6Para2 =>
      'You may Εξαγωγή your data at any Ώρα via the in-app Εξαγωγή feature, and you may permanently Διαγραφή your account and Όλα associated data via Ρυθμίσεις → Διαγραφή Account.';

  @override
  String get tosSection7Title => 'Αποποίηση ευθύνης';

  @override
  String get tosSection7Para1 =>
      'Η ΥΠΗΡΕΣΙΑ ΠΑΡΕΧΕΤΑΙ «ΩΣ ΕΧΕΙ» ΚΑΙ «ΩΣ ΔΙΑΘΕΣΙΜΗ» ΧΩΡΙΣ ΕΓΓΥΗΣΕΙΣ ΟΠΟΙΟΥΔΗΠΟΤΕ ΕΙΔΟΥΣ, ΕΙΤΕ ΡΗΤΕΣ ΕΙΤΕ ΣΙΩΠΗΡΕΣ, ΣΥΜΠΕΡΙΛΑΜΒΑΝΟΜΕΝΩΝ, ΕΝΔΕΙΚΤΙΚΑ, ΤΩΝ ΣΙΩΠΗΡΩΝ ΕΓΓΥΗΣΕΩΝ ΕΜΠΟΡΕΥΣΙΜΟΤΗΤΑΣ, ΚΑΤΑΛΛΗΛΟΤΗΤΑΣ ΓΙΑ ΣΥΓΚΕΚΡΙΜΕΝΟ ΣΚΟΠΟ Ή ΜΗ ΠΑΡΑΒΙΑΣΗΣ.';

  @override
  String get tosSection7Para2 =>
      'We do not warrant that the Service will be uninterrupted, error-free, or secure, or that the AI Διαλογή recommendations will be accurate or appropriate for your specific situation.';

  @override
  String get tosSection8Title => 'Περιορισμός ευθύνης';

  @override
  String tosSection8Para1(String producer) {
    return 'TO THE MAXIMUM EXTENT PERMITTED BY LAW, IN Όχι EVENT SHALL $producer BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES, OR ANY LOSS OF DATA, ARISING OUT OF OR RELATED TO YOUR USE OF (OR INABILITY TO USE) THE SERVICE, WHETHER BASED ON WARRANTY, CONTRACT, TORT, OR ANY Άλλο LEGAL THEORY.';
  }

  @override
  String get tosSection9Title => 'Λήξη';

  @override
  String get tosSection9Para1 =>
      'You may stop using the Service and Διαγραφή your account at any Ώρα via SΡυθμίσεις We may suspend or terminate your access to the Service if you violate these Όροι or if we reasonably believe we are Απαιτείται to do so by law.';

  @override
  String get tosSection9Para2 =>
      'Upon termination, Όλα licenses granted to you will end, and your data will be deleted in accordance with our Απόρρητο Policy.';

  @override
  String get tosSection10Title => 'Changes to These Όροι';

  @override
  String get tosSection10Para1 =>
      'We may update these Όροι from Ώρα to tΏρα We will notify you of material changes via the app or by eEmail Continued use of the Service after changes take effect constitutes acceptance of the revised TΌροι';

  @override
  String get tosSection11Title => 'Επικοινωνία';

  @override
  String get tosSection11Para1 =>
      'Questions Σχετικά these TΌροι Contact us at support@vitalseker.com.';

  @override
  String tosCopyright(int year, String producer, String version) {
    return '© $year $producer. Όλα rights reserved. Version $version.';
  }

  @override
  String get free => 'Δωρεάν';

  @override
  String get pro => 'Pro';

  @override
  String get enterprise => 'Επιχείρηση';

  @override
  String get nA => 'Δεν ισχύει';

  @override
  String get skip => 'Πλοίο';

  @override
  String get next => 'Επόμενο';

  @override
  String get enterVitalSeker => 'Μπείτε στο VitalSeker';

  @override
  String get onboardingTitle1 => 'Γνώρισε το σώμα σου.';

  @override
  String get onboardingDescription1 =>
      'Check any symptom and get reliable medical Ευφυΐα in seconds.';

  @override
  String get onboardingTitle2 => 'Your hΥγεία always with you.';

  @override
  String get onboardingDescription2 =>
      'Store your full medical pΠροφίλ records, and digital insurance cards in one secure, encrypted vault.';

  @override
  String get onboardingTitle3 => 'Works everywhere. Even offline.';

  @override
  String get onboardingDescription3 =>
      'Supported in 40+ languages and counting. Your data stays with you, syncing automatically the moment you\'re Πίσω online.';

  @override
  String get signInSubtitle => 'Συνδεθείτε στον λογαριασμό σας στο VitalSeker';

  @override
  String get signingIn => 'Σύνδεση...';

  @override
  String get emailRequired => 'Email is Απαιτείται';

  @override
  String get enterValidEmail => 'Enter a valid Email';

  @override
  String get enterValidEmailAddress => 'Enter a valid Email address';

  @override
  String get passwordRequired => 'Κωδικός is Απαιτείται';

  @override
  String get enterEmailFirst => 'Please enter your Email address first.';

  @override
  String passwordResetSent(String email) {
    return 'Ο σύνδεσμος επαναφοράς κωδικού πρόσβασης έχει σταλεί στο $email';
  }

  @override
  String get google => 'Google';

  @override
  String get apple => 'Apple';

  @override
  String get male => 'Άνδρας';

  @override
  String get female => 'Γυναίκα';

  @override
  String get other => 'Άλλο';

  @override
  String get creatingAccount => 'Δημιουργία λογαριασμού...';

  @override
  String get joinVitalSeker => 'Join VitalSeker and take control of your Υγεία';

  @override
  String get nameRequired => 'Όνομα is Απαιτείται';

  @override
  String get nameMinChars => 'Όνομα must be at least 2 characters';

  @override
  String get atLeast6Chars => 'Τουλάχιστον 6 χαρακτήρες';

  @override
  String get includeUppercase => 'Περιλάβετε τουλάχιστον ένα κεφαλαίο γράμμα';

  @override
  String get includeLowercase => 'Περιλάβετε τουλάχιστον ένα μικρό γράμμα';

  @override
  String get includeNumber => 'Συμπεριλάβετε τουλάχιστον έναν αριθμό';

  @override
  String get includeSymbol => 'Περιλάβετε τουλάχιστον ένα σύμβολο (!@#\$%^&*)';

  @override
  String get confirmPasswordRequired => 'Please Επιβεβαίωση your Κωδικός';

  @override
  String get optionalDetails => 'Προαιρετικό Details';

  @override
  String get dateOfBirth => 'Ημερομηνία γέννησης';

  @override
  String get selectDateOfBirth => 'Select Ημερομηνία γέννησης';

  @override
  String get selectDateOfBirthHint => 'Select your Ημερομηνία of birth';

  @override
  String get gender => 'Φύλο';

  @override
  String get selectGender => 'Επιλέξτε φύλο';

  @override
  String get bloodType => 'Ομάδα αίματος';

  @override
  String get selectBloodType => 'Επιλέξτε την ομάδα αίματος';

  @override
  String get iAgreeTo => 'I agree to the';

  @override
  String get andText => 'and';

  @override
  String get acceptTermsRequired =>
      'Please accept the Όροι of Service and Απόρρητο Policy to continue.';

  @override
  String get accountCreatedVerifyEmail =>
      'Account created! Please check your Email to verify your account.';

  @override
  String get userFallback => 'Χρήστης';

  @override
  String get viewAll => 'View Όλα';

  @override
  String get failedLoadRecentChecks =>
      'Δεν κατέστη δυνατή η φόρτωση των πρόσφατων επιταγών';

  @override
  String get pullDownRetry => 'Pull down to Επανάληψη';

  @override
  String get goodCondition => 'Σε καλή κατάσταση';

  @override
  String get fairCondition => 'Σε ικανοποιητική κατάσταση';

  @override
  String get needsAttention => 'Χρειάζεται προσοχή';

  @override
  String get poorCondition => 'Κακή κατάσταση';

  @override
  String get critical => 'Κρίσιμο';

  @override
  String get overallHealthIndicator => 'Your overall Υγεία indicator';

  @override
  String get tapForWeeklyInsights => 'Tap for weekly Ευφυΐα';

  @override
  String get aiPoweredTriage60s => 'AI-powered Διαλογή in 60 seconds';

  @override
  String get qrAndMedicalInfo => 'QR και ιατρικές πληροφορίες';

  @override
  String get pastChecksAndVitals => 'Past checks & Ζωτικά σημεία';

  @override
  String get justNow => 'Μόλις τώρα';

  @override
  String hoursAgo(int hours) {
    return '$hours ώρες πριν';
  }

  @override
  String get todayLabel => 'Σήμερα';

  @override
  String get yesterdayLabel => 'Χθες';

  @override
  String daysAgo(int days) {
    return '${days}d πριν';
  }

  @override
  String get symptomCheck => 'Έλεγχος συμπτωμάτων';

  @override
  String get severity => 'Βαθμός σοβαρότητας';

  @override
  String get aiGreeting =>
      'Hello! I\'m VitalSeker AI. How are you feeling tΣήμερα Describe your Συμπτώματα and I\'ll Βοήθεια assess your condition.';

  @override
  String get triageAssessmentIntro =>
      'Based on your sΣυμπτώματα here\'s my assessment:';

  @override
  String get urgencyLabel => 'Επείγον';

  @override
  String get careRecommendationLabel => 'Συμβουλές φροντίδας';

  @override
  String get redFlagsLabel => '⚠️ Προειδοποιητικά σημάδια:';

  @override
  String get recommendationsLabel => 'Συστάσεις:';

  @override
  String get tapForFullAnalysis =>
      'Πατήστε «Προβολή λεπτομερών αποτελεσμάτων» παρακάτω για να δείτε την πλήρη ανάλυση.';

  @override
  String triageErrorMessage(String error) {
    return 'Λυπάμαι, προέκυψε σφάλμα κατά την ανάλυση των συμπτωμάτων σας. Παρακαλώ δοκιμάστε ξανά ή περιγράψτε τα συμπτώματά σας με διαφορετικό τρόπο.\n\nΣφάλμα: $error';
  }

  @override
  String get generalDiscomfort => 'Γενική αδιαθεσία';

  @override
  String get selfCareRecommended => 'Συνιστάται η φροντίδα του εαυτού σας';

  @override
  String get scheduleAppointmentCare => 'Κλείστε ραντεβού';

  @override
  String get visitUrgentCare => 'Επισκεφθείτε το Κέντρο Επείγουσας Περίθαλψης';

  @override
  String get seekEmergencyCare => 'Seek Επείγον Care';

  @override
  String get consultHealthcareProvider =>
      'Συμβουλευτείτε έναν επαγγελματία υγείας';

  @override
  String get mild => 'Ήπια';

  @override
  String get moderate => 'Μέτρια';

  @override
  String get significant => 'Σημαντικό';

  @override
  String get severeLabel => 'Σοβαρό';

  @override
  String get extreme => 'Ακραίο';

  @override
  String get triageResults => 'Διαλογή Results';

  @override
  String urgencyScoreCaption(int score) {
    return 'Δείκτης επείγοντος: $score/100';
  }

  @override
  String get redFlags => 'Προειδοποιητικά σημάδια';

  @override
  String get recommendations => 'Συστάσεις';

  @override
  String get possibleConditions => 'Πιθανές συνθήκες';

  @override
  String get followUpQuestions => 'Ερωτήσεις για περαιτέρω διερεύνηση';

  @override
  String get saving => 'Αποθήκευση...';

  @override
  String get seeDoctorSoon => 'Επισκεφθείτε σύντομα έναν γιατρό';

  @override
  String get emergencyCareNow => 'Επείγον Care Now';

  @override
  String get triageDisclaimer =>
      'Αυτό δεν αποτελεί ιατρική διάγνωση. Συμβουλευτείτε πάντα έναν επαγγελματία υγείας για την κατάλληλη ιατρική συμβουλή.';

  @override
  String get escalateLow1 => 'Συμπτώματα worsen or spread to new body areas';

  @override
  String get escalateLow2 => 'Πυρετός rises above 39°C (102°F)';

  @override
  String get escalateLow3 => 'Όχι improvement after 48 hours of self-care';

  @override
  String get escalateMedium1 => 'Συμπτώματα persist beyond 3 days';

  @override
  String get escalateMedium2 => 'Ο πόνος εντείνεται ή γίνεται αφόρητος';

  @override
  String get escalateMedium3 => 'New red-flag Συμπτώματα appear';

  @override
  String get escalateHigh1 => 'Συμπτώματα rapidly worsen';

  @override
  String get escalateHigh2 =>
      'Εμφανίζεται δυσκολία στην αναπνοή ή αίσθημα σφίξιμου στο στήθος';

  @override
  String get escalateHigh3 =>
      'High Πυρετός (>39°C) that doesn\'t respond to medication';

  @override
  String get escalateEmergency1 => 'Call Επείγον services immediately';

  @override
  String get escalateEmergency2 =>
      'Μην οδηγήσετε εσείς — ζητήστε να σας μεταφέρουν ή καλέστε ασθενοφόρο';

  @override
  String get escalateEmergency3 =>
      'Bring this Διαλογή result and any Φάρμακα you take';

  @override
  String get showQrCode => 'Show Κωδικός QR';

  @override
  String get inactive => 'Ανενεργό';

  @override
  String get knownAllergies => 'Known Αλλεργίες';

  @override
  String get currentMedications => 'Current Φάρμακα';

  @override
  String get chronicConditions => 'Χρόνιες παθήσεις';

  @override
  String get insurance => 'Ασφάλιση';

  @override
  String get qrCode => 'Κωδικός QR';

  @override
  String get exportPdf => 'Εξαγωγή PDF';

  @override
  String get noHealthPassportYet => 'Όχι Υγεία Διαβατήριο Yet';

  @override
  String get completeFirstTriage =>
      'Complete your first Διαλογή to generate your Υγεία Διαβατήριο';

  @override
  String get heightAndWeight => 'Ύψος & Βάρος';

  @override
  String get notSet => 'Δεν έχει οριστεί';

  @override
  String yearsOld(int count) {
    return '$count ετών';
  }

  @override
  String allergiesCount(int count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: '$count Αλλεργίες',
      one: '$count Αλλεργία',
    );
    return '$_temp0';
  }

  @override
  String medicationsCount(int count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: '$count Φάρμακα',
      one: '$count Φάρμακο',
    );
    return '$_temp0';
  }

  @override
  String get healthPassportQr => 'Υγεία Διαβατήριο QR';

  @override
  String get pointQrReader =>
      'Point this at any QR reader to securely Κοινοποίηση your vΖωτικά σημεία';

  @override
  String get download => 'Λήψη';

  @override
  String get share => 'Κοινοποίηση';

  @override
  String get noQrCodeGenerated => 'No Κωδικός QR Generated';

  @override
  String get generateQrCode => 'Generate Κωδικός QR';

  @override
  String get qrGenerationFailed =>
      'Αποτυχία δημιουργίας κωδικού QR. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get expired => 'Έχει λήξει';

  @override
  String validFor(int hours, int minutes) {
    return 'Ισχύει για ${minutes}h ${hours}m';
  }

  @override
  String get emergencySosTitle => 'SOS έκτακτης ανάγκης';

  @override
  String get keepHolding => 'Περίμενε λίγο...';

  @override
  String get nearbyHospitals => 'Νοσοκομεία της περιοχής';

  @override
  String get allergies => 'Αλλεργίες';

  @override
  String get conditions => 'Όροι';

  @override
  String get noMedicalInfoPrefix =>
      'Όχι medical information on file. Update your Προφίλ to';

  @override
  String get addMedicalIdData => 'Προσθήκη medical ID data';

  @override
  String get sosTip =>
      'SOS sends your GPS Τοποθεσία to your Επείγον contacts via SMS. Make sure your contacts are configured in your pΠροφίλ';

  @override
  String get sendingCaps => 'ΑΠΟΣΤΟΛΗ';

  @override
  String get sosFailed => 'SOS ΑΠΟΤΥΧΙΑ';

  @override
  String get sosActive => 'SOS ACTIVE';

  @override
  String get sendingEmergencyAlert => 'Sending Επείγον Alert';

  @override
  String get alertCouldNotBeSent =>
      'Δεν ήταν δυνατή η αποστολή της ειδοποίησης';

  @override
  String get emergencyAlertSent => 'Επείγον Alert Sent';

  @override
  String sendingIn(int seconds) {
    return 'Αποστολή σε $seconds…';
  }

  @override
  String get tryAgain => 'Δοκίμασε ξανά';

  @override
  String get dismiss => 'Απόρριψη';

  @override
  String get euEmergency => 'EU Emergency';

  @override
  String get usEmergency => 'US Emergency';

  @override
  String get samuEmergency => 'SAMU (FR)';

  @override
  String get opensMapsHospitals =>
      'Opens your maps app with Επείγον hospitals nearby';

  @override
  String get liveLocation => 'Live Τοποθεσία';

  @override
  String get acquiringGps => 'Λήψη συντεταγμένων GPS…';

  @override
  String get locationUnavailable => 'Τοποθεσία unavailable';

  @override
  String get contactsNotified => 'Ειδοποιηθέντες επαφές';

  @override
  String contactsNotifiedCount(int count) {
    String _temp0 = intl.Intl.pluralLogic(
      count,
      locale: localeName,
      other: '$count επαφές ενημερώθηκαν μέσω SMS',
      one: '$count επαφή ενημερώθηκε μέσω SMS',
    );
    return '$_temp0';
  }

  @override
  String get failedLoadHistory => 'Δεν κατέστη δυνατή η φόρτωση του ιστορικού';

  @override
  String thisMonthCount(int count) {
    return '$count ΑΥΤΌ ΤΟ ΜΉΝΑ';
  }

  @override
  String get noLogsMatchFilters => 'Όχι logs match your filters';

  @override
  String get tryDifferentSearch => 'Try a different Αναζήτηση or fΦίλτρο';

  @override
  String get searchLogs => 'Αναζήτηση logs...';

  @override
  String get filterGreen => 'Πράσινο';

  @override
  String get filterYellow => 'Κίτρινο';

  @override
  String get filterRed => 'Δίκτυο';

  @override
  String severityCaption(int score) {
    return 'Βαθμός σοβαρότητας: $score/10';
  }

  @override
  String get allSymptoms => 'Όλα Συμπτώματα';

  @override
  String bodyRegionsCaption(String regions) {
    return 'Περιοχές του σώματος: $regions';
  }

  @override
  String durationCaption(String duration) {
    return 'Διάρκεια: $duration';
  }

  @override
  String aiRecommendationCaption(String recommendation) {
    return 'Πρόταση τεχνητής νοημοσύνης: $recommendation';
  }

  @override
  String get viewFullTriageResult => 'View Full Διαλογή Result';

  @override
  String notesCaption(String notes) {
    return 'Σημειώσεις: $notes';
  }

  @override
  String get export30DayReport => 'Εξαγωγή 30-day Report (PPro';

  @override
  String get weeklyBreakdown => 'Εβδομαδιαία ανάλυση';

  @override
  String get proAnalysis => 'Επαγγελματική ανάλυση';

  @override
  String get yourHealthThisWeek => 'Your Υγεία this week';

  @override
  String scoreChangePts(int change) {
    return '$change pts';
  }

  @override
  String get trendAnalysis => 'Ανάλυση τάσεων';

  @override
  String get symptomFrequency4w => 'ΣΥΧΝΟΤΗΤΑ ΣΥΜΠΤΩΜΑΤΩΝ (4W)';

  @override
  String get chartHigh => 'Υψηλό';

  @override
  String get chartAvg => 'Μέσος όρος';

  @override
  String get chartLow => 'Χαμηλή';

  @override
  String get tipSleepTitle => 'Επέκταση του βαθύ ύπνου';

  @override
  String get tipSleepBody =>
      'Η θερμοκρασία του σώματός σας μειώθηκε προς το τέλος αυτής της εβδομάδας. Προσπαθήστε να διατηρήσετε το δωμάτιο σε χαμηλότερη θερμοκρασία (65°F) για να επιταχύνετε την έναρξη των φάσεων του βαθιού ύπνου.';

  @override
  String get tipHydrationTitle => 'Προληπτική ενυδάτωση';

  @override
  String get tipHydrationBody =>
      'Στα αρχεία καταγραφής του απογεύματος εντοπίστηκαν δείκτες ήπιας αφυδάτωσης. Μεταφέρετε το 40% της ημερήσιας πρόσληψης νερού σας πριν από τις 10 π.μ., προκειμένου να σταθεροποιήσετε τον μεταβολικό ρυθμό.';

  @override
  String get tipActivityTitle => 'Ρυθμός δραστηριότητας';

  @override
  String get tipActivityBody =>
      'Οι εξάρσεις στον πόνο των αρθρώσεων σχετίζονται με απότομες αυξήσεις της έντασης. Φροντίστε να κάνετε μια δυναμική προθέρμανση 10 λεπτών πριν προχωρήσετε σε καρδιοπροπόνηση πέρα από τη ζώνη 2.';

  @override
  String get personalizedFocus => 'Εξατομικευμένη προσέγγιση';

  @override
  String get refreshingAiInsights =>
      'Ενημέρωση των αναλύσεων τεχνητής νοημοσύνης…';

  @override
  String get generateNewInsights => 'Generate New Ευφυΐα';

  @override
  String get symptoms => 'Συμπτώματα';

  @override
  String get avgSeverity => 'Μέση σοβαρότητα';

  @override
  String get scoreChange => 'Αλλαγή βαθμολογίας';

  @override
  String get checkBackMondayOrGenerate =>
      'Όχι Ευφυΐα generated yet. Check Πίσω on Monday for your weekly AI Υγεία summary, or tap below to generate one now.';

  @override
  String get upgradeProInsightsFull =>
      'Αναβάθμιση to Pro to unlock AI-powered weekly Υγεία iΕυφυΐα Get personalized recommendations and trend analysis every Monday.';

  @override
  String proPlanMonthly(double price) {
    return 'Pro Plan - \$$price/μήνα';
  }

  @override
  String get weeklyInsightsUnlimitedTriage =>
      'Weekly iΕυφυΐα unlimited Διαλογή';

  @override
  String get couldNotLoadInsights => 'Could not load Ευφυΐα';

  @override
  String couldNotLaunchCall(String phoneNumber) {
    return 'Δεν ήταν δυνατή η πραγματοποίηση κλήσης προς τον αριθμό $phoneNumber';
  }

  @override
  String get medicalDisclaimer =>
      'Οι πληροφορίες αυτές δεν αποτελούν ιατρική διάγνωση. Το VitalSeker δεν υποκαθιστά έναν εξειδικευμένο επαγγελματία υγείας.';

  @override
  String get medicalDisclaimerShort =>
      'Ο Seker παρέχει γενικές οδηγίες, όχι διάγνωση. Πάντα να συμβουλεύεστε γιατρό.';

  @override
  String get sekerGreeting =>
      'Γεια! Είμαι ο Seker, ο AI βοηθός υγείας σας. Είμαι εδώ για να σας βοηθήσω να κατανοήσετε τα συμπτώματά σας. Τι σας απασχολεί σήμερα; Μπορείτε να πληκτρολογήσετε ή να πατήστε το μικρόφωνο.';

  @override
  String get aiChatTriage => 'Συνομιλία με AI Seker';

  @override
  String get chatWithSeker => 'Συνομιλία με τον Seker';

  @override
  String get chatWithSekerDesc =>
      'Μιλήστε με τον AI βοηθό υγείας μας σε πραγματικό χρόνο';

  @override
  String get poweredByGlm => 'Powered by GLM-4';

  @override
  String get aiTriageIn90Seconds => 'AI-powered Διαλογή in 90 seconds';

  @override
  String get vitalValueOutOfRange => 'Τιμή εκτός του αναμενόμενου εύρους';

  @override
  String get vitalRangeHintHeartRate =>
      'Ο καρδιακός ρυθμός πρέπει να κυμαίνεται μεταξύ 30 και 220 παλμών ανά λεπτό';

  @override
  String get vitalRangeHintBloodPressure =>
      'Blood pressure should be between 60-250 (sΣυστολική and 40-150 (dΔιαστολική';

  @override
  String get vitalRangeHintTemperature =>
      'Η θερμοκρασία του σώματος πρέπει να κυμαίνεται μεταξύ 30 και 45 °C';

  @override
  String get vitalRangeHintOxygen =>
      'Το οξυγόνο στο αίμα πρέπει να κυμαίνεται μεταξύ 50 και 100 %';

  @override
  String get vitalRangeHintGlucose =>
      'Η γλυκόζη στο αίμα πρέπει να κυμαίνεται μεταξύ 20 και 600 mg/dL';

  @override
  String get vitalRangeHintWeight => 'Βάρος should be between 2 and 500 kg';

  @override
  String get vitalRangeHintHeight => 'Ύψος should be between 30 and 250 cm';

  @override
  String get familyProfilesProOnly =>
      'Οικογένεια Profiles is a Pro feature. Αναβάθμιση to Προσθήκη Οικογένεια members.';

  @override
  String get exportProOnly =>
      'PDF Εξαγωγή is a Pro feature. Αναβάθμιση to generate reports.';

  @override
  String get triageLimitReached =>
      'You have reached your Δωρεάν monthly Διαλογή limit (3). Αναβάθμιση to Pro for unlimited triages.';

  @override
  String get monthlyTriageLimit => '3 triages per month (Δωρεάν plan)';

  @override
  String get viewQrCode => 'View Κωδικός QR';

  @override
  String get shareMedicalId => 'Share Ιατρικό ID';

  @override
  String translationTooLong(int max) {
    return 'Το κείμενο είναι πολύ μακρύ (μέγιστο $max χαρακτήρες). Παρακαλώ συντομεύστε το και δοκιμάστε ξανά.';
  }

  @override
  String get profileFieldsSaveFailed =>
      'Account created, but Προαιρετικό Προφίλ fields (Ημερομηνία of birth, gender, blood type) couldn\'t be saved. You can Επεξεργασία them later in Προφίλ > Επεξεργασία PΠροφίλ';

  @override
  String get privacyPolicyTitle => 'Απόρρητο Policy';

  @override
  String get privacyLastUpdated => 'Τελευταία ενημέρωση: Μάρτιος 2025';

  @override
  String privacyIntro(String appName) {
    return 'At $appName, your Απόρρητο is paramount. This Απόρρητο Policy explains how we collect, use, store, and protect your personal and health-related data. By using our services, you agree to the practices described below.';
  }

  @override
  String get privacySectionDataCollection => '1. Συλλογή δεδομένων';

  @override
  String get privacyDataCollectionBody =>
      'We collect the following categories of data: • Personal Information: NΌνομα Email address, Τηλέφωνο number, Ημερομηνία of birth, and gender. • Υγεία Data: Blood type, aΑλλεργίες chronic conditions, mΦάρμακα vital signs, symptom logs, and Διαλογή results. • Επείγον Contacts: Names, Τηλέφωνο numbers, and relationships of your designated contacts. • Device Data: Device type, operating system, and app version for compatibility and sΥποστήριξη • Usage Data: Feature interactions and anonymized analytics to improve our services. We only collect data that is necessary for providing our Υγεία companion services. You have full control over what information you provide.';

  @override
  String get privacySectionDataStorage =>
      '2. Αποθήκευση και κρυπτογράφηση δεδομένων';

  @override
  String get privacyDataStorageBody =>
      'Your data is stored using industry-leading security measures:\n\n• Encryption at Rest: All data stored in our databases is encrypted using AES-256 encryption.\n• Encryption in Transit: All data transmitted between your device and our servers uses TLS 1.3 encryption.\n• Health Passport: Your health passport data is encrypted with a unique key derived from your credentials.\n• Κωδικός QR Sharing: Shared health data via QR codes is encrypted and time-limited.\n• Infrastructure: Our servers are hosted in SOC 2 Type II certified data centers with 24/7 monitoring.\n\nWe do not store payment card information. All payment processing is handled by certified third-party providers.';

  @override
  String get privacySectionGdpr => '3. Συμμόρφωση με τον ΓΚΠΔ';

  @override
  String privacyGdprBody(String appName) {
    return '$appName is fully compliant with the General Data Protection Regulation (GDPR): • Lawful Basis: We process your data based on your explicit consent and contractual necessity. • Data Minimization: We only collect and process data that is strictly necessary. • Purpose Limitation: Your data is used only for the purposes for which it was collected. • Right to Access: You can request a complete copy of your personal data at any tΏρα • Right to Rectification: You can update or correct your data through the app sΡυθμίσεις • Right to Erasure: You can request complete deletion of your account and data. • Right to Portability: You can Εξαγωγή your data in a machine-readable format. • Data Processing Agreements: Όλα third-party processors have signed DPAs. • Cross-Border Transfers: Data is processed within the EU/EEA unless explicit consent is given otherwise.';
  }

  @override
  String get privacySectionYourRights => '4. Τα δικαιώματά σας';

  @override
  String get privacyRightsBody =>
      'You have the following rights regarding your data: • Access: View Όλα your personal and Υγεία data within the app or request a data eΕξαγωγή • Correction: Επεξεργασία your Προφίλ information at any Ώρα through Επεξεργασία PΠροφίλ • Deletion: Request account deletion through Ρυθμίσεις > Data & Απόρρητο > Διαγραφή Account. • Restriction: Limit how certain data is processed by adjusting your notification and sharing preferences. • Objection: Object to specific data processing activities by contacting our Data Protection Officer. • Withdrawal of Consent: You may withdraw consent at any Ώρα without affecting the lawfulness of prior processing. To exercise any of these rights, contact us at privacy@vitalseker.com or through the in-app Υποστήριξη feature.';

  @override
  String get privacySectionContactUs => '5. Επικοινωνήστε μαζί μας';

  @override
  String privacyContactBody(String producer) {
    return 'If you have any questions or concerns Σχετικά this Απόρρητο Policy or our data practices, please contact us: • EEmail privacy@vitalseker.com • SΥποστήριξη support@vitalseker.com • Data Protection Officer: dpo@vitalseker.com • Address: $producer, Data Protection Office We aim to respond to Όλα privacy-related inquiries within 30 days.';
  }

  @override
  String privacyCopyright(String producer) {
    return '© 2025 $producer. Όλα rights reserved.';
  }

  @override
  String get chooseFromGallery => 'Επιλέξτε από τη συλλογή φωτογραφιών';

  @override
  String get takePhoto => 'Τράβα μια φωτογραφία';

  @override
  String get removePhoto => 'Αφαίρεση Photo';

  @override
  String get avatarUpdated => 'Το προφίλ ενημερώθηκε!';

  @override
  String get avatarUploadFailed =>
      'Δεν κατέστη δυνατή η μεταφόρτωση του προφίλ. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get avatarRemoved => 'Το avatar καταργήθηκε.';

  @override
  String get avatarRemoveFailed =>
      'Δεν κατέστη δυνατή η διαγραφή του προφίλ. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get profileUpdatedSuccessfully => 'Προφίλ updated successfully!';

  @override
  String get profileUpdateFailed =>
      'Δεν κατέστη δυνατή η ενημέρωση του προφίλ. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get editProfileTitle => 'Επεξεργασία Προφίλ';

  @override
  String errorPrefix(String error) {
    return 'Error: $error';
  }

  @override
  String get personalInformation => 'Προσωπικά στοιχεία';

  @override
  String get measurements => 'Μετρήσεις';

  @override
  String get heightCm => 'Ύψος (cm)';

  @override
  String get weightKg => 'Βάρος (kg)';

  @override
  String get emergencyContactSection => 'Επείγον Contact';

  @override
  String get contactName => 'Contact Όνομα';

  @override
  String get phoneNumber => 'Τηλέφωνο Number';

  @override
  String get relationshipHint => 'π.χ. σύζυγος, γονέας, αδελφός/αδελφή';

  @override
  String get addAllergy => 'Προσθήκη Allergy';

  @override
  String get noAllergiesAdded => 'Όχι Αλλεργίες added';

  @override
  String get addCondition => 'Προσθήκη Condition';

  @override
  String get noConditionsAdded => 'Όχι conditions added';

  @override
  String get saveChanges => 'Αποθήκευση Changes';

  @override
  String get healthTitle => 'Υγεία';

  @override
  String get weeklyInsightsTooltip => 'Weekly Ευφυΐα';

  @override
  String get yourHealthScore => 'Your Υγεία Score';

  @override
  String get scoreDescriptionGreat =>
      'Your Υγεία metrics are looking great! Keep it up.';

  @override
  String get scoreDescriptionGood =>
      'Καλή πρόοδος. Υπάρχουν μερικά σημεία που χρήζουν προσοχής.';

  @override
  String get scoreDescriptionModerate =>
      'Some Υγεία metrics need improvement. Consider our recommendations.';

  @override
  String get scoreDescriptionLow =>
      'Υπάρχουν διάφοροι τομείς που απαιτούν προσοχή. Παρακαλούμε να συμβουλευτείτε έναν επαγγελματία υγείας.';

  @override
  String get scoreDescriptionCritical =>
      'Συνιστάται άμεση φροντίδα. Παρακαλείστε να συμβουλευτείτε γιατρό.';

  @override
  String get riskFactors => 'Παράγοντες κινδύνου';

  @override
  String allergyCount(int count) {
    return '$count Αλλεργίες';
  }

  @override
  String chronicConditionCount(int count) {
    return '$count Χρόνιες παθήσεις';
  }

  @override
  String get noRiskFactors => 'Όχι risk factors identified';

  @override
  String get recentTriageResults => 'Recent Διαλογή Results';

  @override
  String get noTriageResults => 'Όχι Διαλογή results yet';

  @override
  String get recommendedActions => 'Συνιστώμενες ενέργειες';

  @override
  String get actionScheduleCheckup => 'Κλείστε ραντεβού για προληπτικό έλεγχο';

  @override
  String get actionScheduleCheckupDesc =>
      'Your Υγεία score suggests it\'s Ώρα for a medical review.';

  @override
  String get actionLogVitals => 'Log Your Ζωτικά σημεία';

  @override
  String get actionLogVitalsDesc =>
      'Track your blood pressure, heart rate, and Άλλο key metrics.';

  @override
  String get actionRunSymptomCheck => 'Εκτέλεση ελέγχου συμπτωμάτων';

  @override
  String get actionRunSymptomCheckDesc =>
      'Use AI Διαλογή to assess any Συμπτώματα you\'re experiencing.';

  @override
  String get actionImproveSleep => 'Βελτίωση της ποιότητας του ύπνου';

  @override
  String get actionImproveSleepDesc =>
      'Ο ποιοτικός ύπνος είναι απαραίτητος για την ανάρρωση και τη λειτουργία του ανοσοποιητικού συστήματος.';

  @override
  String get actionStayActive => 'Μείνετε δραστήριοι';

  @override
  String get actionStayActiveDesc =>
      'Regular exercise helps maintain cardiovascular hΥγεία';

  @override
  String get viewWeeklyInsights => 'View Weekly Ευφυΐα';

  @override
  String minutesAgo(int minutes) {
    return '$minutes λεπτά πριν';
  }

  @override
  String weeksAgo(int weeks) {
    return '$weeks εβδομάδες πριν';
  }

  @override
  String get failedToLoadVitals => 'Αποτυχία φόρτωσης των ζωτικών ενδείξεων';

  @override
  String get noVitalsYet => 'Όχι Ζωτικά σημεία Yet';

  @override
  String get startLoggingVitalsPrompt =>
      'Start logging your vital signs to track your Υγεία over Ώρα';

  @override
  String get logFirstVital => 'Καταγράψτε τα πρώτα σας ζωτικά σημεία';

  @override
  String get day => 'Αυτό';

  @override
  String get week => 'Εβδομάδα';

  @override
  String get month => 'Μήνας';

  @override
  String get noData => 'Όχι data';

  @override
  String get trendUp => 'UP';

  @override
  String get trendDown => 'DOWN';

  @override
  String get trendStable => 'STABLE';

  @override
  String get logVitalTitle => 'Καταγραφή ζωτικών δεικτών';

  @override
  String get vitalTypeLabel => 'ΒΑΣΙΚΟΣ ΤΥΠΟΣ';

  @override
  String get valueLabel => 'ΑΞΙΑ';

  @override
  String get systolic => 'Συστολική';

  @override
  String get diastolic => 'Διαστολική';

  @override
  String get dateTimeLabel => 'Ημερομηνία & Ώρα';

  @override
  String get notesOptionalLabel => 'Σημειώσεις (OΠροαιρετικό';

  @override
  String get notesHint => 'Προσθήκη any Σημειώσεις Σχετικά this reading...';

  @override
  String saveVitalType(String vitalType) {
    return 'Αποθήκευση $vitalType';
  }

  @override
  String vitalSavedSuccessfully(String vitalType) {
    return 'Το $vitalType αποθηκεύτηκε με επιτυχία';
  }

  @override
  String get vitalSaveFailed =>
      'Δεν κατέστη δυνατή η αποθήκευση των βασικών στοιχείων. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get vitalsHistoryTitle => 'Ζωτικά σημεία Ιστορικό';

  @override
  String get range7Days => '7D';

  @override
  String get range1Month => '1M';

  @override
  String get range3Months => '3M';

  @override
  String get range6Months => '6M';

  @override
  String get range1Year => '1Y';

  @override
  String noVitalTypeData(String vitalType) {
    return 'Όχι $vitalType Data';
  }

  @override
  String get noReadingsForPeriod =>
      'Δεν βρέθηκαν μετρήσεις για την επιλεγμένη χρονική περίοδο';

  @override
  String get singleReading => '1 ανάγνωση';

  @override
  String get average => 'Μέσος όρος';

  @override
  String get min => 'I';

  @override
  String get max => 'Μαξ';

  @override
  String get latest => 'Τελευταία νέα';

  @override
  String get readingsLabel => 'ΑΝΑΓΝΩΣΕΙΣ';

  @override
  String get value => 'Τιμή';

  @override
  String get source => 'Πηγή';

  @override
  String showingReadingsCount(int total) {
    return 'Εμφάνιση 20 από $total μετρήσεις';
  }

  @override
  String get addMedicationTitle => 'Προσθήκη Medication';

  @override
  String get medicationNameLabel => 'Medication Όνομα';

  @override
  String get dosageLabel => 'Δοσολογία';

  @override
  String get unitLabel => 'Μονάδα';

  @override
  String get frequencyLabel => 'Συχνότητα';

  @override
  String get startDateLabel => 'Start Ημερομηνία';

  @override
  String get setEndDate => 'Set end Ημερομηνία';

  @override
  String get endDateLabel => 'End Ημερομηνία';

  @override
  String get doseTimesLabel => 'ΩΡΕΣ ΧΟΡΗΓΗΣΗΣ';

  @override
  String doseNumber(int number) {
    return 'Δόση $number';
  }

  @override
  String get notesOptional => 'Σημειώσεις (oΠροαιρετικό';

  @override
  String get remindersLabel => 'Υπενθυμίσεις';

  @override
  String get saveMedication => 'Αποθήκευση Medication';

  @override
  String get medicationAddedSuccessfully =>
      'Το φάρμακο προστέθηκε με επιτυχία!';

  @override
  String get medicationAddFailed =>
      'Δεν κατέστη δυνατή η προσθήκη φαρμάκου. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get addAppointmentTitle => 'Προσθήκη Appointment';

  @override
  String get doctorNameLabel => 'Όνομα γιατρού';

  @override
  String get specialtyLabel => 'Ειδικότητα';

  @override
  String get selectSpecialtyHint => 'Select Ειδικότητα';

  @override
  String get locationOptional => 'Τοποθεσία / Address (oΠροαιρετικό';

  @override
  String get reminderLabel => 'Υπενθύμιση';

  @override
  String get saveAppointment => 'Αποθήκευση Appointment';

  @override
  String get appointmentScheduledSuccessfully =>
      'Το ραντεβού προγραμματίστηκε με επιτυχία!';

  @override
  String get appointmentScheduleFailed =>
      'Δεν κατέστη δυνατό να προγραμματιστεί το ραντεβού. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get specialtyCardiologist => 'Καρδιολόγος';

  @override
  String get specialtyDermatologist => 'Δερματολόγος';

  @override
  String get specialtyEndocrinologist => 'Ενδοκρινολόγος';

  @override
  String get specialtyGeneralPractice => 'Γενική Ιατρική';

  @override
  String get specialtyNeurologist => 'Νευρολόγος';

  @override
  String get specialtyOphthalmologist => 'Οφθαλμίατρος';

  @override
  String get specialtyOrthopedic => 'Ορθοπεδική';

  @override
  String get specialtyPediatrician => 'Παιδίατρος';

  @override
  String get specialtyPsychiatrist => 'Ψυχίατρος';

  @override
  String get specialtyOther => 'Άλλο';

  @override
  String get recordsLoadFailed =>
      'Δεν κατέστη δυνατή η φόρτωση των εγγραφών. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get notAvailable => 'N/A';

  @override
  String get editRecordTitle => 'Επεξεργασία Record';

  @override
  String get addMedicalRecordTitle => 'Προσθήκη Medical Record';

  @override
  String get titleLabel => 'Τίτλος';

  @override
  String get typeLabel => 'Τύπος';

  @override
  String get recordTypeLabResults => 'Αποτελέσματα εργαστηριακών εξετάσεων';

  @override
  String get recordTypePrescriptions => 'Οδηγίες χρήσης';

  @override
  String get recordTypeImaging => 'Απεικόνιση';

  @override
  String get recordTypeOther => 'Άλλο';

  @override
  String get descriptionLabel => 'Περιγραφή';

  @override
  String get recordUpdated => 'Η εγγραφή ενημερώθηκε!';

  @override
  String get recordAdded => 'Η εγγραφή προστέθηκε!';

  @override
  String get recordUpdateFailed =>
      'Δεν κατέστη δυνατή η ενημέρωση της εγγραφής.';

  @override
  String get recordAddFailed => 'Δεν κατέστη δυνατή η προσθήκη εγγραφής.';

  @override
  String get deleteRecordTitle => 'Διαγραφή Record';

  @override
  String deleteRecordConfirm(String title) {
    return 'Are you sure you want to Διαγραφή \"$title\"? This cannot be undone.';
  }

  @override
  String get recordDeleted => 'Η εγγραφή διαγράφηκε.';

  @override
  String get recordDeleteFailed =>
      'Δεν κατέστη δυνατή η διαγραφή της εγγραφής.';

  @override
  String get medicalRecordsTitle => 'Ιατρικά αρχεία';

  @override
  String get searchRecordsHint => 'Αναζήτηση records...';

  @override
  String get noRecordsFound => 'Όχι records found';

  @override
  String get tapToAddRecord => 'Tap + to Προσθήκη a medical record';

  @override
  String get untitled => 'Untitled';

  @override
  String get moreOptions => 'Περισσότερες επιλογές';

  @override
  String triageStepOf(Object current, Object total) {
    return 'Βήμα $total από $current';
  }

  @override
  String get triageQ1Title => 'What are your main Συμπτώματα tΣήμερα';

  @override
  String get triageQ1Subtitle =>
      'Select Όλα that apply. This helps our AI analyze your condition accurately.';

  @override
  String get triageQ2Title => 'How long have you had these sΣυμπτώματα';

  @override
  String get triageQ2Subtitle =>
      'Select the option that best describes when your Συμπτώματα started.';

  @override
  String get triageQ3Title => 'What is your Ηλικία and biological sex?';

  @override
  String get triageQ3Subtitle =>
      'This information helps calibrate the Διαλογή assessment.';

  @override
  String get triageQ4Title => 'Any known conditions or mΦάρμακα';

  @override
  String get triageQ4Subtitle =>
      'Pre-filled from your pΠροφίλ Επεξεργασία if needed.';

  @override
  String get triageQ5Title => 'Any additional nΣημειώσεις';

  @override
  String get triageQ5Subtitle =>
      'Describe any Άλλο details Σχετικά your Συμπτώματα or situation.';

  @override
  String get triageSeverityLabel => 'Συνολική σοβαρότητα';

  @override
  String get triageSeverityMild => 'Ήπια';

  @override
  String get triageSeveritySevere => 'Σοβαρό';

  @override
  String get severityVeryMild => 'Πολύ ήπιο';

  @override
  String get severityMild => 'Ήπια';

  @override
  String get severityModerate => 'Μέτρια';

  @override
  String get severityDistracting => 'Αποσπά την προσοχή';

  @override
  String get severitySevere => 'Σοβαρό';

  @override
  String get severityUnbearable => 'Αφόρητο';

  @override
  String get durationToday => 'Σήμερα (started tΣήμερα';

  @override
  String get duration1To3Days => '1-3 ημέρες';

  @override
  String get duration4To7Days => '4-7 ημέρες';

  @override
  String get duration1To2Weeks => '1-2 εβδομάδες';

  @override
  String get durationMoreThan2Weeks => 'Πάνω από 2 εβδομάδες';

  @override
  String get age => 'Ηλικία';

  @override
  String get ageHint => 'Enter your Ηλικία';

  @override
  String get yearsSuffix => 'χρόνια';

  @override
  String get biologicalSex => 'Βιολογικό φύλο';

  @override
  String get conditionsHint => 'π.χ. διαβήτης, υπέρταση, άσθμα';

  @override
  String get medicationsHint => 'π.χ. Λισινοπρίλη 10 mg, Μετφορμίνη 500 mg';

  @override
  String get triageSummary => 'Περίληψη';

  @override
  String get analyzeWithAi => 'Ανάλυση με AI';

  @override
  String get back => 'Πίσω';

  @override
  String get triageFailed =>
      'Η ανάλυση διαλογής απέτυχε. Παρακαλώ δοκιμάστε ξανά.';

  @override
  String get symptomFever => 'Πυρετός';

  @override
  String get symptomHeadache => 'Πονοκέφαλος';

  @override
  String get symptomCough => 'Βήχας';

  @override
  String get symptomShortnessOfBreath => 'Σύντομη αναπνοή';

  @override
  String get symptomFatigue => 'Κόπωση';

  @override
  String get symptomDizziness => 'Ζάλη';

  @override
  String get symptomNausea => 'Ναυτία';

  @override
  String get symptomChills => 'Ρίγη';

  @override
  String get symptomMuscleAche => 'Μυϊκός πόνος';

  @override
  String get symptomInsomnia => 'Αϋπνία';

  @override
  String get symptomChestPain => 'Πόνος στο στήθος';

  @override
  String get symptomAbdominalPain => 'Κοιλιακός πόνος';

  @override
  String get symptomSoreThroat => 'Πονόλαιμος';

  @override
  String get symptomRunnyNose => 'Ρινική καταρροή';

  @override
  String get symptomVomiting => 'Εμετός';

  @override
  String get symptomDiarrhea => 'Διάρροια';

  @override
  String get symptomRash => 'Εξάνθημα';

  @override
  String get symptomJointPain => 'Πόνος στις αρθρώσεις';

  @override
  String get purchaseCancelled => 'Η αγορά ακυρώθηκε.';

  @override
  String get noPurchasesToRestore =>
      'Όχι previous purchases found to rΕπαναφορά';

  @override
  String get failedToCompleteOnboarding =>
      'Η ολοκλήρωση της εισαγωγής απέτυχε. Ελέγξτε τη σύνδεσή σας και δοκιμάστε ξανά.';

  @override
  String get fieldRequired => 'Αυτό το πεδίο είναι υποχρεωτικό';

  @override
  String get somethingWentWrong => 'Κάτι πήγε στραβά. Δοκιμάστε ξανά.';

  @override
  String get securitySettingsOnThisPage =>
      'Οι ρυθμίσεις ασφαλείας βρίσκονται σε αυτή τη σελίδα';

  @override
  String get vitalDeleted => 'Η μέτρηση διαγράφηκε.';

  @override
  String get failedToDeleteVital =>
      'Αποτυχία διαγραφής μέτρησης. Δοκιμάστε ξανά.';

  @override
  String get deleteVitalTitle => 'Διαγραφή μέτρησης';

  @override
  String deleteVitalConfirm(String value) {
    return 'Διαγραφή αυτής της μέτρησης: $value;';
  }

  @override
  String get vitalScore => 'Vital Score';

  @override
  String get scoreLabelExcellent => 'Excellent';

  @override
  String get scoreLabelGood => 'Good';

  @override
  String get scoreLabelFair => 'Fair';

  @override
  String get scoreLabelPoor => 'Poor';

  @override
  String get scoreLabelCritical => 'Critical';

  @override
  String get aboutSekerAi => 'About Seker AI';

  @override
  String get sekerAiName => 'Seker AI';

  @override
  String get sekerAiSubtitle => 'Your AI Health Assistant';

  @override
  String get sekerWhatIsTitle => 'What is Seker?';

  @override
  String get sekerWhatIsBody =>
      'Seker is an AI-powered health assistant built into VitalSeker. Seker is an expert in biology, human health, psychology, and the human body. Seker provides general health guidance, symptom analysis, and emotional support.';

  @override
  String get sekerCapabilitiesTitle => 'What can Seker do?';

  @override
  String get sekerCapabilitiesBody =>
      '• Understand your symptoms and ask follow-up questions\n• Provide general health guidance and coaching\n• Help manage stress, anxiety, and emotional well-being\n• Auto-detect and save health information you share\n• Respond in your language (40+ supported)\n• Accept voice notes and file uploads';

  @override
  String get sekerHowToUseTitle => 'How to use Seker';

  @override
  String get sekerHowToUseBody =>
      '1. Type your message or tap the mic to speak\n2. Seker will ask questions to understand your situation\n3. Seker provides guidance and recommendations\n4. You can upload prescriptions, lab results, or images\n5. Health information you share is auto-saved to your profile';

  @override
  String get sekerSafetyTitle => 'Important Safety Information';

  @override
  String get sekerSafetyBody =>
      '• Seker provides general guidance, NOT a medical diagnosis\n• Always consult a professional doctor for proper diagnosis\n• Seker does NOT recommend specific medications or dosages\n• For emergencies, call 112 or 911 immediately\n• Seker only discusses health, biology, and psychology\n• Your conversations are private and secure';

  @override
  String get backToChat => 'Back to Chat';

  @override
  String get switchToLightMode => 'Switch to light mode';

  @override
  String get switchToDarkMode => 'Switch to dark mode';

  @override
  String get comprehensiveHealthReport => 'COMPREHENSIVE HEALTH REPORT';

  @override
  String get generated => 'Generated';

  @override
  String get range => 'Range';

  @override
  String get medicationsAndAllergies => 'Medications & Allergies';

  @override
  String get symptomsAndTriageLog => 'Symptoms & Triage Log';

  @override
  String get nameLabel => 'Name';

  @override
  String get emailLabel => 'Email';

  @override
  String get dateOfBirthLabel => 'Date of Birth';

  @override
  String get bloodTypeLabel => 'Blood Type';

  @override
  String get vitalScoreLabel => 'Vital Score';

  @override
  String get chronicConditionsLabel => 'Chronic Conditions';

  @override
  String get allergiesLabel => 'Allergies';

  @override
  String get medicationsLabel => 'Medications';

  @override
  String get symptomsLabel => 'Symptoms';

  @override
  String get severityLabel => 'Severity';

  @override
  String get noneRecorded => 'None recorded';

  @override
  String get thisDocumentDisclaimer =>
      'This document is generated by VitalSeker AI Health Companion and does not constitute a medical diagnosis. Always consult a qualified healthcare professional.';

  @override
  String get failedToEmailPdf => 'Failed to email PDF. Please try again.';

  @override
  String get failedToExportPdf => 'Failed to export PDF. Please try again.';

  @override
  String get generatePdf => 'Generate PDF';

  @override
  String get inAppPurchasesNotAvailable =>
      'In-app purchases are not available on this device.';

  @override
  String get couldNotLoadPlans =>
      'Could not load available plans. Please check your connection and try again.';

  @override
  String planNotAvailable(String plan) {
    return 'The $plan plan is not available for purchase right now. Please try again later.';
  }

  @override
  String get documentScannedAndUploaded =>
      'Document scanned and uploaded successfully';

  @override
  String get couldNotUploadFile =>
      'Could not upload file. Please check your connection and try again.';

  @override
  String fileTooLarge(String size) {
    return 'File is too large ($size MB). Maximum allowed size is 10 MB.';
  }

  @override
  String get fileTypeNotAllowed =>
      'File type not allowed. Please upload a JPG, PNG, PDF, DOC, or DOCX file.';

  @override
  String get appointmentTimeMustBeFuture =>
      'Appointment time must be in the future. Please pick a later time.';

  @override
  String get enterValidNumber => 'Enter a valid number';

  @override
  String get doseMustBePositive => 'Dose must be greater than 0';

  @override
  String get doseTooHigh => 'Dose seems too high — please check';

  @override
  String get overrideRateLimit => 'Override Rate Limit?';

  @override
  String get overrideRateLimitMessage =>
      'You sent an SOS less than 60 seconds ago. If this is a NEW emergency (not a repeat of the same one), you can override the rate limit and send another alert.\n\nWarning: overriding will send a second SMS to all your emergency contacts. Only do this if you have a genuinely new emergency.';

  @override
  String get overrideAndSend => 'Override & Send';

  @override
  String get areYouSafeConfirm =>
      'Are you safe? Your emergency contacts will be notified that the alert has been resolved. If you are NOT safe, tap \"Cancel\" and call emergency services directly.';

  @override
  String get yesImSafe => 'Yes, I\'m safe';

  @override
  String get qrSavedToPhotos =>
      'QR code saved. Use the share sheet to save to Photos or Files.';

  @override
  String get qrSavedToDownloads => 'QR code saved to Downloads.';

  @override
  String get qrSavedToAppStorage => 'QR code saved to app storage.';

  @override
  String get shareMedicalPassport => 'Share Medical Passport?';

  @override
  String get shareMedicalPassportMessage =>
      'This QR code grants access to your medical information (blood type, allergies, chronic conditions, medications, emergency contacts). Anyone who receives it can scan it and view your health data.\n\nOnly share it with people you trust — doctors, nurses, or emergency contacts. Do not post it on social media.\n\nThe QR code expires in 24 hours.';

  @override
  String get showMore => 'Show more';

  @override
  String get noAllergiesRecorded =>
      'No allergies recorded. Add yours in Edit Profile so emergency responders are aware.';

  @override
  String get noMedicationsRecorded =>
      'No medications recorded. Add your current prescriptions so they appear on your passport.';

  @override
  String get noChronicConditionsRecorded =>
      'No chronic conditions recorded. Add any ongoing conditions (diabetes, hypertension, etc.) in Edit Profile.';

  @override
  String get speechNotAvailable =>
      'Speech recognition not available. Please type instead.';

  @override
  String get couldNotStartRecording =>
      'Could not start recording. Check microphone permissions.';

  @override
  String get pleaseSignInToUpload => 'Please sign in to upload files.';

  @override
  String get uploading => 'Uploading';

  @override
  String get fileAttached => 'File attached';

  @override
  String get couldNotPickFile => 'Could not pick file';

  @override
  String get typeYourMessage => 'Type your message...';

  @override
  String get online => 'Online';

  @override
  String get offline => 'Offline';

  @override
  String get typing => 'typing...';

  @override
  String get stop => 'Stop';

  @override
  String get recording => 'Recording...';

  @override
  String get noTextFoundInImage =>
      'No text found in the image. Try a clearer photo, or type the text manually. Note: non-Latin scripts may not be recognized on all devices.';

  @override
  String documentScanned(int count) {
    return 'Document scanned! $count characters extracted. Tap Translate.';
  }

  @override
  String get translationPdfReady => 'Translation PDF ready!';

  @override
  String get couldNotGeneratePdf => 'Could not generate PDF';

  @override
  String get translationContainsNonLatin =>
      'Note: the translation contains characters that may not render in the PDF. If the PDF shows blank boxes, please copy the text from the screen instead.';

  @override
  String get callEmergencyNow => 'Call 112 / 911 Now';

  @override
  String get couldNotOpenDialer =>
      'Could not open the phone dialer. Please call 112 or your local emergency number manually.';

  @override
  String get sosRateLimitMessage =>
      'You just sent an SOS. Please wait 60 seconds before sending another — emergency services may already be responding.';

  @override
  String get sosDeliveryFailedMessage =>
      'Failed to send alert. Please call emergency services directly (112 / 911).';

  @override
  String get sosRateLimitRetryMessage =>
      'SOS rate limit: please wait 60 seconds before retrying.';

  @override
  String get sosQueued => 'SOS QUEUED';

  @override
  String get emergencyAlertQueued => 'Emergency Alert Queued';

  @override
  String get emergencyContact => 'Emergency Contact';

  @override
  String get tapToViewQrCode => 'Tap to view QR Code';

  @override
  String get translation => 'Medical Translation';

  @override
  String get translationDesc =>
      'Translate medical terms into 40+ languages with DeepL';

  @override
  String get medicalRecordsDesc =>
      'Store and manage prescriptions, lab results, and imaging files';

  @override
  String get vitalsTrackingTitle => 'Vitals Tracking';

  @override
  String get vitalsTrackingDesc =>
      'Track heart rate, blood pressure, temperature, and more';

  @override
  String get medicationRemindersTitle => 'Medication Reminders';

  @override
  String get medicationRemindersDesc =>
      'Never miss a dose with smart medication reminders';

  @override
  String get appointmentManagerTitle => 'Appointment Manager';

  @override
  String get appointmentManagerDesc => 'Schedule and track doctor appointments';

  @override
  String get offlineModeTitle => 'Offline Mode';

  @override
  String get offlineModeDesc =>
      'Access your health passport and history without internet';

  @override
  String get languagesTitle => '41 Languages';

  @override
  String get languagesDesc =>
      'Full app localization in 41 languages including African languages';

  @override
  String get primaryCarePhysician => 'Primary Care Physician';
}
